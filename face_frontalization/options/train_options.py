...
class TrainOptions:
	...
	def initialize(self):
		self.parser.add_argument('--exp_dir', default= './experiments_ffhq/tune_28_june_parameters_lr0.00001_wnorm_ignor_lpips0.6_id_loss2_cropignor_pretrained_bothdatatmppp+', type=str, help='Path to experiment output directory')
		...
		self.parser.add_argument('--workers', default=12, type=int, help='Number of train dataloader workers')
		self.parser.add_argument('--test_workers', default=12, type=int, help='Number of test/inference dataloader workers')
		...
		self.parser.add_argument('--start_from_latent_avg', default = True, help='Whether to add average latent vector to generate codes from encoder.')
		self.parser.add_argument('--learn_in_w', action='store_true', help='Whether to learn in w space instead of w+')

		self.parser.add_argument('--lpips_lambda', default=0.06, type=float, help='LPIPS loss multiplier factor')
		self.parser.add_argument('--id_lambda', default=2, type=float, help='ID loss multiplier factor')
		self.parser.add_argument('--l2_lambda', default=0.001, type=float, help='L2 loss multiplier factor')
		self.parser.add_argument('--w_norm_lambda', default=0.000000005, type=float, help='W-norm loss multiplier factor')
		self.parser.add_argument('--lpips_lambda_crop', default=0.0000005, type=float, help='LPIPS loss multiplier factor for inner image region')
		self.parser.add_argument('--l2_lambda_crop', default=0.00000001, type=float, help='L2 loss multiplier factor for inner image region')
		...
		self.parser.add_argument('--val_interval', default=1500, type=int, help='Validation interval')
